...
class Chain(object):
    ...
    def add_blocks(self, blocks, insert_point=None):
        if not blocks:
            return
        # 添加区块的功能 (深拷贝*)
    ...
    def ShowStructureWithGraphviz(self, dot=None, chain_id=None, prpchain=None, vote_counts=None,
                                  max_votes_per_height=None):
        if dot is None:
        ...
        fork_list = []
        while blocktmp:
            if blocktmp.isGenesis:
                node_name = f'B0_{chain_id}' if chain_id else 'B0'
                dot.node(node_name, label=f'{node_name}\nIndex: {blocktmp.index}', shape='rect', color='black',
                         fontsize='20')
            else:
                block_color = 'red' if blocktmp.isAdversaryBlock else 'blue'
                votes_label = f'\nVotes: {vote_counts.get(blocktmp.name, 0)}' if vote_counts else ''
                if blocktmp.index != 1:
                    formatted_pouw_opt_x = ', '.join([f"{x:.2f}" for x in blocktmp.blockhead.pouw_opt_x])
                    dot.node(blocktmp.name,
                             label=f'{blocktmp.name}\nIndex: {blocktmp.index}\n{votes_label}\nMiner: {blocktmp.blockhead.miner}\ncontent: {blocktmp.blockhead.content}'
                             # f'\nblockhash: {blocktmp.blockhash}\nprehash: {blocktmp.blockhead.prehash}\nheight: {blocktmp.height}\ntimestamp: {blocktmp.blockhead.timestamp}'
                                   f'\nparent: {blocktmp.parentblock.name if blocktmp.parentblock else "Genesis"}'
                                   f'\nsolution: [{formatted_pouw_opt_x}]'
                                   f'\nvalue: {blocktmp.blockhead.pouw_opt_y}'
                                   f'\ntimestamp:{blocktmp.blockhead.timestamp}'
                                   f'\nbenchmark_x:{blocktmp.blockhead.benchmark_x}'
                                   f'\nbenchmark_y:{blocktmp.blockhead.benchmark_y}'
                                   f'\nnonce1:{getattr(blocktmp.blockhead, "nonce", "N/A")}\nnonce2:{getattr(blocktmp.blockhead, "nonce2", "N/A")}\nnonce3:{getattr(blocktmp.blockhead, "nonce3", "N/A")}'
                                   f'\nvton:{[block.name for block in blocktmp.vtonprp] if blocktmp.vtonprp else "[]"}',
                             shape='rect', color=block_color)
                else:
                    dot.node(blocktmp.name,
                             label=f'{blocktmp.name}\nIndex: {blocktmp.index}\n{votes_label}\nMiner: {blocktmp.blockhead.miner}\nparent: {blocktmp.parentblock.name if blocktmp.parentblock else "Genesis"}',
                                   # f'\nblockhash: {blocktmp.blockhash}\nprehash: {blocktmp.blockhead.prehash}\nheight: {blocktmp.height}\ntimestamp: {blocktmp.blockhead.timestamp}',
                             shape='rect', color=block_color)
                if blocktmp.parentblock:
                    dot.edge(blocktmp.parentblock.name, blocktmp.name)
                # if chain_id.startswith('vtchain') and blocktmp.parentblock:
                #     for block in blocktmp.vtonprp:
                #         dot.edge(blocktmp.name, block.name, color='green', style='dashed', constraint='false')
                if max_votes_per_height is not None:
                    for block_height, winning_block in max_votes_per_height.items():
                        votes_label = f'\nVotes: {vote_counts.get(winning_block.name, 0)}'
                        dot.node(winning_block.name,
                                 label=f'{winning_block.name}\nIndex: {winning_block.index}{votes_label}\nMiner: {winning_block.blockhead.miner}',
                                 shape='rect', color='green', style='filled')
            list_tmp = copy.copy(blocktmp.next)
            if list_tmp:
                blocktmp = list_tmp.pop(0)
                fork_list.extend(list_tmp)
    ...
    def CalculateStatistics(self, rounds, honest_miners: list, adver_ids: list[int], confirm_delay: int,
        ...
        # print(f"honest_cp_1: {honest_cp.name}")
        ...
        #     print(f"honest_cp_2: {honest_cp.name}")
    ...
    # PoUW投票相关方法
    def has_voted(self, height):
        """检查是否已经对指定高度投票"""
        return height in self.votes

    def vote(self, height, block):
        """对指定高度的区块投票"""
        self.votes[height] = block

    def get_vote(self, height):
        """获取指定高度的投票"""
        return self.votes.get(height, None)

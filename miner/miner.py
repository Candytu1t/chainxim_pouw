...
class Miner(object):
    ...
    def launch_consensus(self, input, round, tasks):
        '''开始共识过程
        ...
        new_msgs, msg_available = self.consensus.consensus_process(
            self._isAdversary, input, round, tasks)
    ...
    def BackboneProtocol(self, round, tasks):
        chain_update_prp, update_index, chain_update_vt = self.consensus.local_state_update()
        input = I(round, self.input_tape)  # I function
        new_msgs, msg_available = self.launch_consensus(input, round, tasks)
    ...
    def clear_tapes(self):
        ...
        self.consensus._receive_tape_tx = []
        self.consensus._receive_tape_prp = []
        self.consensus._receive_tape_vt = []
